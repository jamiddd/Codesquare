...
class ChatFragment: PagerListFragment<Message, MessageViewHolder2<Message>>() {
    ...
    private fun initChatBottom(chatChannelId: String) {
        ...
        fab?.size = SIZE_MINI
        fab?.setImageDrawable(ContextCompat.getDrawable(requireContext(), R.drawable.ic_round_keyboard_double_arrow_down_24))
        ...
        val params2 = fab?.layoutParams as ConstraintLayout.LayoutParams
        params2.endToEnd = binding.pagerRoot.id
        params2.bottomToBottom = binding.pagerRoot.id
        params2.startToStart = binding.pagerRoot.id
        params2.horizontalBias = 1.0f
        params2.setMargins(convertDpToPx(8), 0, convertDpToPx(8), convertDpToPx(64))

        fab?.layoutParams = params2
